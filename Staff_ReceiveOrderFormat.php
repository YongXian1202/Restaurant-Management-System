<?php
	...
	if (mysqli_num_rows($result5) <= 0) {
		...
		if (mysqli_affected_rows($conn) > 0) {
			...
			echo "<script>alert('Successful send the order!');window.location.href='Staff_TableOrder.php';</script>";
		...
		if (mysqli_affected_rows($conn) > 0) {
			echo "<script>alert('Successful add the order!');window.location.href='Staff_TableOrder.php';</script>";
			unset($_SESSION["shopping_item"]);
		} else {
			echo "<script>alert('Please enter at least 1 item!');window.history.go(-1);</script>";
		}
