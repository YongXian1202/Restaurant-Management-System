...
<form action="GenerateReportMonthly.php" method="POST">
...
$year=date("Y",$time);
                ...
                echo "<center><h3>$monthFull $year</h3></center>";
                




                $sql ="SELECT item.Item_Name, SUM(report_usage.Quantity)AS Total_Quantity, SUM(report_usage.Amount) AS Total_Amount, YEAR(report_usage.Date), MONTH(report_usage.Date) FROM item LEFT JOIN report_usage ON report_usage.Item_ID = item.Item_ID AND Year(report_usage.Date)='".$year."' AND MONTH(report_usage.Date)='".$month."' GROUP BY item.Item_ID ORDER BY SUM(report_usage.Quantity) DESC, SUM(report_usage.Amount) DESC;";
                $result=mysqli_query($conn, $sql);
...
$sql = "SELECT SUM(Quantity) AS Total_Quantity, SUM(Amount) AS Total_Amount, DAYNAME(Date) AS Day, MONTHNAME(Date) AS Month, YEAR(Date) AS Year FROM report_usage WHERE YEAR(Date) = '".$year."' GROUP BY MONTH(Date), YEAR(Date) ;";
$result=mysqli_query($conn, $sql);
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        while($row = mysqli_fetch_array($result)){
            if($row['Total_Amount'] === NULL){
                $row['Total_Amount'] =  number_format(0.00,2); //2 decimal places
            }
                else{
                    $row['Total_Amount'] =  number_format($row['Total_Amount'],2); 
                }
