...
<form action="GenerateReportYearly.php" method="POST">
...
$year=date("Y",$time);
                ...
                echo "<center><h3>$year</h3></center>";
                




                $sql ="SELECT item.Item_Name, SUM(year_usage.Quantity)AS Total_Quantity, SUM(year_usage.Amount) AS Total_Amount FROM item LEFT JOIN year_usage ON year_usage.Item_ID = item.Item_ID AND year_usage.Year = '".$year."' GROUP BY item.Item_ID ORDER BY SUM(year_usage.Quantity) DESC, SUM(year_usage.Amount) DESC;";
                $result=mysqli_query($conn, $sql);
...
$sql = "SELECT SUM(Quantity) AS Total_Quantity, SUM(Amount) AS Total_Amount, Year FROM year_usage GROUP BY Year;";
$result=mysqli_query($conn, $sql);

    if(mysqli_num_rows($result) > 0)
    {

        echo "<center><h3>Yearly Report Summary in $year</h3></center>";
